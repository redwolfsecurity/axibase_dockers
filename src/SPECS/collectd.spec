...
# Enable write_kafka by default, because --with flag doesn't work else
# It is disabled by default in collectd original spec
...
%if 0%{?fedora} || 0%{?rhel} >= 7
%{__install} -Dp -m0644 contrib/systemd.collectd-axibase.service %{buildroot}%{_unitdir}/collectd-axibase.service
%else
%{__install} -Dp -m0755 contrib/redhat/init.d-collectd %{buildroot}%{_initrddir}/collectd-axibase
%endif
...
%post
%if 0%{?fedora} || 0%{?rhel} >= 7
%systemd_post collectd-axibase.service
%else
...
%preun
%if 0%{?fedora} || 0%{?rhel} >= 7
%systemd_preun collectd-axibase.service
%else
...
%postun
%if 0%{?fedora} || 0%{?rhel} >= 7
%systemd_postun_with_restart collectd-axibase.service
%else
...
%if 0%{?fedora} || 0%{?rhel} >= 7
%{_unitdir}/collectd-axibase.service
%else
%{_initrddir}/collectd-axibase
%endif
%{_sbindir}/collectd
